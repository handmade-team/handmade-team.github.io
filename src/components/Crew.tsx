...
const CrewCard = forwardRef<HTMLDivElement, { member: CrewMember }>(({ member }, ref) => {
  return (
    <div
      ref={ref}
      className='reveal'
    ...
    >
      ...
      >
        <div style={{ fontSize: '1.3rem', fontWeight: 600, color: '#2c2c2a' }}>{member.name}</div>
        {member.link && (
          <a
            href={member.link}
            target='_blank'
          ...
          >
            <svg
              ...
              style={{ color: '#666', transition: 'color 0.3s ease' }}
              onMouseEnter={e => {
                (e.currentTarget as SVGElement).style.color = '#fff';
              }}
              onMouseLeave={e => {
                (e.currentTarget as SVGElement).style.color = '#666';
              }}
            >
      ...
      <div style={{ fontSize: '0.9rem', color: '#999' }}>{member.role}</div>
    </div>
  );
});
